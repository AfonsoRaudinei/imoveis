...
export function normalizeInput(data) {
  return {
    ...
    configuracao: {
      ...
      horizonte_meses: Math.floor(parseNumber(data?.configuracao?.horizonte_meses) || 0),
      intervalo_exibicao: Math.floor(parseNumber(data?.configuracao?.intervalo_exibicao) || 0),
      filtro_temporal_ativo: Boolean(data?.configuracao?.filtro_temporal_ativo ?? false),
      meses_exibicao:
        data?.configuracao?.meses_exibicao ??
        data?.configuracao?.meses_filtro ??
        data?.configuracao?.meses_ativos ??
        [],
